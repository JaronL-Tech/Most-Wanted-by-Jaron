...
function searchByTraits(people) {
  const traitsTOSearchForString = validatedPrompt(
    "Please enter one of the listed traits of the person you are searching for.",
    ["eyeColor", "height", "weight", "gender", "occupation", "quit"]
  );
...
function mainMenu(person, people) {
  ...
  switch (mainMenuUserActionChoice) {
    case "info":
      //! TODO
      let PersonInfo = PersonInfo(person);
...
function findPersonDescendants(person, people) {
  ...
  const descendants = people.filter(
    (person) => person.parents[0] === personid || person.parents[1] === personid
  );
  return descendants;
}

function findPersonEyeColor(people) {
  let eyeColorValue = validatedPrompt(
    "What eye color would you like to search for? ",
    ["brown", "blue", "hazel", "black", "green"]
  );
...
function findPersonGender(people) {
  let GenderValue = validatedPrompt(
    "What Gender would you like to search for? ",
    ["Male", "Female"]
  );
...
function findPersonheight(people) {
  let HeightValue = parseInt(
    ...
    [
      71, 65, 76, 62, 70, 72, 66, 59, 69, 76, 74, 66, 58, 62, 61, 74, 71, 70,
      63, 70, 67, 67,
    ]
  );
...

{
  function findPersonweight(people) {
    let WeightValue = parseInt(
      "Enter weight? ",
      [
        175, 162, 250, 115, 207, 256, 170, 137, 199, 205, 118, 179, 156, 235,
        112, 184, 249, 187, 241, 110, 100, 100,
      ]
    );
...
}
{
  function findPersonOccupation(people) {
    let OccupationValue = validatedPrompt(
